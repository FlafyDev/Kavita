...
export class BookReaderComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // @ViewChild('stickyBottom', {static: false}) stickyTopElemRef!: ElementRef<HTMLDivElement>;
    ...
    this.accountService.currentUser$.pipe(take(1)).subscribe((user) => {
      if (user) {
        ...
        this.settingsForm.addControl(
          ...
          new FormControl(user.preferences.bookReaderFontFamily, [])
        );
    ...
    fromEvent(this.reader.nativeElement, 'scroll')
      ...
      .subscribe((event) => {
        ...
        const intersectingEntries = Array.from(
          this.readingSectionElemRef.nativeElement.querySelectorAll(
  ...
  ngOnDestroy(): void {
    ...
    if (
      bodyNode !== undefined &&
      bodyNode !== null &&
      this.originalBodyColor !== undefined
    ) {
  ...
  //     }
  //     // this.pageStyles = {
  //     //   'font-family': this.user.preferences.bookReaderFontFamily,
  //     //   'font-size': this.user.preferences.bookReaderFontSize + '%',
  //     //   'margin-left': margin,
  //     //   'margin-right': margin,
  //     //   'line-height': this.user.preferences.bookReaderLineSpacing + '%',
  //     // };
      ...
      .subscribe((content) => {
        ...
        setTimeout(() => {
          ...
          requestAnimationFrame(
            () =>
              (this.topOffset =
                this.stickyTopElemRef.nativeElement?.getBoundingClientRect().height)
          );
  ...
  //   let val = parseInt(
  //     this.pageStyles['font-size'].substr(
  //       0,
  //       this.pageStyles['font-size'].length - 1
  //     ),
  //     10
  //   );
  ...
  //   let cleanedValue = this.pageStyles['margin-left']
  //     .replace('%', '')
  //     .replace('!important', '')
  //     .trim();
   ...
   */
  updateReaderStyles = (newStyle: BookStyle) => {
    // this.accountService
    //   .updatePreferences(this.user.preferences)
    ...
    if (this.styleElement == null) {
      ...
      this.readingHtml.nativeElement.appendChild(this.styleElement);
    }

    this.styleElement.innerHTML = `
        .book-content {
            font-size: ${newStyle.fontSize}% !important;
        }
          
        .book-content > * {
            line-height: ${newStyle.lineHeight}% !important;
        }
    `;
  ...
  toggleQuickSettings(opened?: boolean) {
    if (opened === undefined) {
      this.quickSettingsOpen = !this.quickSettingsOpen;
    } else {
      this.quickSettingsOpen = opened;
    }
  }
